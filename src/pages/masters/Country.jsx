...
export default function Country() {
  ...
  // ── Search ────────────────────────────────────────────────────────────────
  const handleSearch = () => {
    const c = searchCode.trim().toLowerCase();
    const n = searchName.trim().toLowerCase();
    setFiltered(
      allData.filter(
        (row) =>
          (!c || (row.Countrycode || "").toLowerCase().includes(c)) &&
          (!n || (row.Countryname || "").toLowerCase().includes(n)),
      ),
    );
    setPage(1);
  };
  ...
  // ── Duplicate check (Countrycode on blur) ─────────────────────────────────
